...
export const profile = {
  ...
  about: [
    ...
    "I'm a business graduate (BBA, Khulna University) who chose engineering anyway. I went professional with PHP in 2008 and started freelancing on oDesk — now Upwork — in 2009, working directly with founders and teams around the world. The PHP years were good to me: Laravel, Symfony, CodeIgniter, WordPress, plenty of shipped product. But somewhere along the way I fell hard for Python, and Django and DRF became the tools I reached for first. Since then I've never stopped collecting languages — Go and Node.js are daily drivers now, and Clojure, Elixir, Kotlin, Rust and others have all earned a place in my heart.",
    "Today my work lives on the backend, at the architecture and distributed-systems end of it. For 15+ years I've built systems that have to stay up while serving millions of people a month — email and SMS delivery platforms, real-time analytics pipelines, job-scraping fleets, API gateways, lead routers. The problems I enjoy most are the unglamorous-but-critical ones: concurrency and async (Python's asyncio internals and the GIL, Go's goroutines), real-time systems on Django Channels and ASGI, exactly-once data pipelines, and clean REST API design. I care about why a thing works, not just that it does.",
    "I have strong opinions held loosely. I reach for boring, proven technology when the goal is to ship something that lasts — Celery, Redis, MongoDB, Postgres, MySQL — matching each tool to the job's real risks rather than chasing hype. I deploy what I build. These days that's mostly AWS — ECS with Docker — though over the years I've also run Linux with nginx, uWSGI and Daphne, serverless on AWS Lambda via Zappa and Apex, and deployments across Heroku and Rackspace. I've also been hired as a third-party code reviewer through Upwork — auditing PHP, Python, Node.js, Go and Java codebases for clients.",
